...
from src.orchestrator.human_in_loop import (
    HumanReviewManager,
    ReviewPoint,
    HumanFeedback,
    FeedbackType,
...
class iReDevCLI:
    ...
    def initialize_system(self) -> bool:
        ...
        try:
            ...
            self.review_manager = HumanReviewManager(
                artifact_pool=self.artifact_pool, event_bus=self.event_bus
            )
    ...
    async def cmd_start(self, args):
        ...
        domain = (
            args.domain or input("Domain (e.g., web, mobile, enterprise): ").strip()
        )
        if not domain:
            domain = "general"
        ...
        target_env = (
            args.environment
            or input("Target environment (e.g., cloud, on-premise, hybrid): ").strip()
        )
        if not target_env:
            target_env = "cloud"
    ...
    def cmd_status(self, args):
        ...
        if args.all:
            ...
            self.print_info(
                "Use --session-id or --all to specify which session to show"
            )
            return 1
    ...
    def _monitor_process(self, session_id: str, refresh_interval: int = 5) -> int:
        ...
        self.print_info(
            f"Refreshing every {refresh_interval} seconds (Press Ctrl+C to stop)"
        )
    ...
    def _submit_review(self, args) -> int:
        ...
        feedback_type = feedback_type_map.get(
            args.type, FeedbackType.MODIFICATION_REQUEST
        )
    ...
    def _interactive_review(self, session_id: Optional[str] = None) -> int:
        ...
        for review in pending_reviews:
            ...
            if artifact:
                ...
                print(
                    json.dumps(artifact.content, indent=2)[:500] + "..."
                    if len(str(artifact.content)) > 500
            ...
            if section:
                ...
                if section == "agents":
                    ...
                    for name, agent_config in config.agents.items():
                        ...
                        print(
                            f"  Knowledge Modules: {', '.join(agent_config.knowledge_modules)}"
                        )
                    ...
                    print(
                        f"  Notification Channels: {', '.join(hil.notification_channels)}"
                    )
    ...
    def _reset_config(self) -> int:
        ...
        confirm = (
            input("Are you sure you want to reset configuration to defaults? (y/N): ")
            .strip()
            .lower()
        )
        if confirm == "y":
    ...
    def _list_artifacts(
        self, session_id: Optional[str] = None, artifact_type: Optional[str] = None
    ) -> int:
        """List artifacts."""
    ...
    def _export_artifacts(
        self, session_id: Optional[str] = None, output_path: str = "artifacts_export"
    ) -> int:
        """Export artifacts."""
...
Examples:
  ...
  %(prog)s config --show agents
    ...
    start_parser = subparsers.add_parser(
        "start", help="Start a new requirement development process"
    )
    start_parser.add_argument("--project", help="Project name")
    start_parser.add_argument(
        "--domain", help="Project domain (e.g., web, mobile, enterprise)"
    )
    start_parser.add_argument(
        "--stakeholders", help="Comma-separated list of stakeholders"
    )
    start_parser.add_argument(
        "--environment", help="Target environment (e.g., cloud, on-premise)"
    )
    ...
    start_parser.add_argument(
        "--monitor", action="store_true", help="Monitor the process after starting"
    )
    ...
    status_parser.add_argument(
        "--all", action="store_true", help="Show all active sessions"
    )

    # Monitor command
    monitor_parser = subparsers.add_parser(
        "monitor", help="Monitor a process in real-time"
    )
    ...
    review_parser.add_argument(
        "--list", action="store_true", help="List pending reviews"
    )
    review_parser.add_argument("--session-id", help="Filter reviews by session ID")
    review_parser.add_argument(
        "--submit", action="store_true", help="Submit review feedback"
    )
    review_parser.add_argument("--review-id", help="Review ID for submission")
    review_parser.add_argument(
        "--type",
        choices=["approve", "reject", "modify", "clarify"],
        default="modify",
        help="Feedback type",
    )
    ...
    config_parser.add_argument(
        "--section",
        choices=["agents", "knowledge", "human_in_loop"],
