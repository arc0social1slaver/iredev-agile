...
class KnowledgeDrivenAgent(BaseAgent, EventHandler):
    ...
    def _load_knowledge_modules(self) -> None:
        ...
        logger.info(
            f"Loaded {len(loaded_modules)} knowledge modules for agent {self.name}"
        )
    ...
    def generate_with_cot(
        ...
        if not process:
            logger.error(
                f"Failed to create reasoning process with template: {reasoning_template}"
            )
            return {
        ...
        response = self.generate_response(
            [
                self.llm.format_messages("system", enhanced_prompt),
                self.llm.format_messages("user", prompt),
            ]
        )
    ...
    def _create_enhanced_prompt(
        self, original_prompt: str, context: Dict[str, Any], process: CoTProcess
    ) -> str:
        """
...
"""
    ...
    def start_session(
        self, session_id: str, coordinator: Optional[AgentCoordinator] = None
    ) -> None:
        """
        ...
        if self.event_bus and self.session_id:
            self.event_bus.publish_agent_started(
                agent_name=self.name, session_id=self.session_id, source=self.name
            )
    ...
    def _handle_task_completed_notification(self, message: Message) -> None:
        ...
        logger.info(
            f"Agent {message.sender} completed task: {task_info.get('description', 'Unknown')}"
        )
    ...
    def notify_agent(
        self, recipient: str, notification_type: str, data: Dict[str, Any]
    ) -> None:
        """
    ...
    def update_coordinator_state(
        self, load_level: Optional[float] = None, **metadata
    ) -> None:
        """
    ...
    def submit_task_to_coordinator(
        self,
        task_type: str,
        description: str,
