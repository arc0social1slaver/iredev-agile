...
class ClaudeLLM(BaseLLM):
    ...
    def __init__(
        self, api_key: str, model: str, rate_limits: Optional[Dict[str, Any]] = None
    ):
        ...
        self.rate_limiter = RateLimiter(
            ...
            input_tokens_per_minute=limits.get(
                "input_tokens_per_minute", default_limits["input_tokens_per_minute"]
            ),
            output_tokens_per_minute=limits.get(
                "output_tokens_per_minute", default_limits["output_tokens_per_minute"]
            ),
            input_token_price_per_million=limits.get(
                "input_token_price_per_million",
                default_limits["input_token_price_per_million"],
            ),
            output_token_price_per_million=limits.get(
                "output_token_price_per_million",
                default_limits["output_token_price_per_million"],
            ),
        )
    ...
    def _count_messages_tokens(
        self, messages: List[Dict[str, str]], system_message: Optional[str] = None
    ) -> int:
        ...
        claude_messages = [
            self._convert_to_claude_message(msg)
            for msg in messages
            if msg["role"] != "system"
        ]
        ...
        try:
            ...
            count = self.client.beta.messages.count_tokens(
                model=self.model, messages=claude_messages, system=system_content
            )
            return count.input_tokens
        except Exception as e:
            # Log the error but don't fail
            ...
            total_tokens += 10 * len(
                claude_messages
            )  # Add ~10 tokens per message for formatting
    ...
    def format_messages(self, role: str, content: str) -> Dict[str, str]:
