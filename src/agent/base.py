...
class BaseAgent(ABC):
    """Base class for all agent in the requirements development process"""
    ...
    def _initialize_llm(
        ...
        if ("api_key" not in llm_config or not llm_config["api_key"]) and (
            llm_config["type"] not in ["huggingface", "local"]
        ):
    ...
    def add_to_memory(self, role: str, content: str):
        ...
        self.memory.append(self.llm.format_messages(role, content))
    ...
    def refresh_memory(self, new_memory: list[Dict[str, Any]]) -> None:
        ...
        """
        self.memory = [
            self.llm.format_messages(msg["role"], msg["content"]) for msg in new_memory
        ]
