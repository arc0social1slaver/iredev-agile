...
class BaseAgent(ABC):
    ...
    def __init__(self, name: str, config_path: Optional[str] = None):
        ...
        agent_section = (
                raw_config.get("agents", {}).get(name) or
                raw_config.get("iredev", {}).get("agents", {}).get(name) or {}
        )
        llm_cfg = (
                raw_config.get("agent_llms", {}).get(name) or
                raw_config.get("llm", {})
        )
    ...
    def react(self, state: Dict[str, Any], task: str) -> Dict[str, Any]:
        ...
        for step in range(self.max_react_iterations):
            ...
            if result.should_return:
                logger.debug("[%s] Tool '%s' triggered early return.", self.name, action)
                break
