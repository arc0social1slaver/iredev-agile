...
class EndUserAgent(KnowledgeDrivenAgent):
    ...
    def __init__(self, config_path: Optional[str] = None, **kwargs):
        ...
        self.nfr_categories = self.config.get('nfr_categories', [
            'performance', 'security', 'usability', 'reliability', 
            'scalability', 'accessibility', 'maintainability'
        ])
        ...
        logger.info(f"Initialized EndUserAgent with {len(knowledge_modules)} knowledge modules")
...
"""
    ...
    def create_user_personas(self, domain: str, context: Dict[str, Any]) -> List[UserPersona]:
        """
        ...
        personas = self._parse_personas_from_response(cot_result["response"], domain, context)
    ...
    def generate_user_scenarios(self, personas: List[UserPersona], 
                               system_context: Dict[str, Any]) -> List[UserScenario]:
        """
    ...
    def identify_pain_points(self, scenarios: List[UserScenario], 
                           current_system_info: Optional[Dict[str, Any]] = None) -> List[PainPoint]:
        """
    ...
    def define_non_functional_requirements(self, scenarios: List[UserScenario], 
                                         pain_points: List[PainPoint],
                                         system_constraints: Optional[Dict[str, Any]] = None) -> List[NonFunctionalRequirement]:
        """
        ...
        """
        logger.info(f"Generating NFRs from {len(scenarios)} scenarios and {len(pain_points)} pain points")
        ...
        for category in self.nfr_categories:
            ...
            relevant_scenarios = self._filter_scenarios_for_nfr_category(scenarios, category)
            relevant_pain_points = self._filter_pain_points_for_nfr_category(pain_points, category)
            ...
            category_nfrs = self._parse_nfrs_from_response(
                cot_result["response"], category, relevant_scenarios, relevant_pain_points
            )
    ...
    def _parse_personas_from_response(self, response: str, domain: str, 
                                    context: Dict[str, Any]) -> List[UserPersona]:
    ...
    def _create_persona_from_dict(self, persona_dict: Dict[str, Any], domain: str) -> Optional[UserPersona]:
        ...
        try:
            return UserPersona(
                ...
                demographics=self._parse_demographics(persona_dict.get('demographics', '')),
                goals=self._parse_list_field(persona_dict.get('goals', '')),
                pain_points=self._parse_list_field(persona_dict.get('pain_points', '')),
                technical_proficiency=persona_dict.get('technical_proficiency', 'intermediate'),
                context_of_use=f"{domain} domain usage",
                motivations=self._parse_list_field(persona_dict.get('motivations', '')),
                frustrations=self._parse_list_field(persona_dict.get('frustrations', '')),
                preferred_interaction_style=persona_dict.get('interaction_style', 'intuitive'),
                accessibility_needs=self._parse_list_field(persona_dict.get('accessibility_needs', ''))
            )
    ...
    def _create_default_personas(self, domain: str, context: Dict[str, Any]) -> List[UserPersona]:
    ...
    def _parse_scenarios_from_response(self, response: str, persona_id: str, 
                                     system_context: Dict[str, Any]) -> List[UserScenario]:
    ...
    def _create_scenario_from_dict(self, scenario_dict: Dict[str, Any], persona_id: str) -> Optional[UserScenario]:
        ...
        try:
            return UserScenario(
                ...
                preconditions=self._parse_list_field(scenario_dict.get('preconditions', '')),
                steps=self._parse_list_field(scenario_dict.get('steps', '')),
                expected_outcome=scenario_dict.get('expected_outcome', 'Task completed successfully'),
                success_criteria=self._parse_list_field(scenario_dict.get('success_criteria', '')),
    ...
    def _create_default_scenarios(self, persona_id: str, system_context: Dict[str, Any]) -> List[UserScenario]:
        ...
        return [
            UserScenario(
                ...
                steps=["Login to system", "Navigate to main dashboard", "Access required features"],
                expected_outcome="User successfully accesses system functionality",
                success_criteria=["Login successful", "Dashboard loads", "Features accessible"],
                frequency="daily",
                importance="high",
                complexity="simple",
                environment="office"
    ...
    def _parse_pain_points_from_response(self, response: str, persona_id: str, 
                                       scenarios: List[UserScenario]) -> List[PainPoint]:
        ...
        for line in lines:
            ...
            if line.startswith('PAIN POINT:') or line.startswith('Title:'):
                if current_pain_point:
                    pain_point = self._create_pain_point_from_dict(current_pain_point, persona_id)
                    if pain_point:
    ...
    def _create_pain_point_from_dict(self, pain_point_dict: Dict[str, Any], persona_id: str) -> Optional[PainPoint]:
        ...
        try:
            return PainPoint(
                ...
                description=pain_point_dict.get('description', 'User experiences difficulty'),
  ...
  def _filter_scenarios_for_nfr_category(self, scenarios: List[UserScenario], category: str) -> List[UserScenario]:
    ...
    def _filter_pain_points_for_nfr_category(self, pain_points: List[PainPoint], category: str) -> List[PainPoint]:
    ...
    def _parse_nfrs_from_response(self, response: str, category: str, 
                                scenarios: List[UserScenario], 
                                pain_points: List[PainPoint]) -> List[NonFunctionalRequirement]:
        ...
        for line in lines:
            ...
            if line.startswith('NFR:') or line.startswith('Title:'):
                if current_nfr:
                    nfr = self._create_nfr_from_dict(current_nfr, category, scenarios, pain_points)
                    if nfr:
                        nfrs.append(nfr)
    ...
    def _create_nfr_from_dict(self, nfr_dict: Dict[str, Any], category: str,
                            scenarios: List[UserScenario], 
                            pain_points: List[PainPoint]) -> Optional[NonFunctionalRequirement]:
        ...
        try:
            return NonFunctionalRequirement(
                ...
                description=nfr_dict.get('description', f'System must meet {category} standards'),
                rationale=nfr_dict.get('rationale', f'Required for {category} compliance'),
                priority=nfr_dict.get('priority', 'medium'),
                measurable_criteria=self._parse_list_field(nfr_dict.get('measurable_criteria', '')),
                acceptance_criteria=self._parse_list_field(nfr_dict.get('acceptance_criteria', '')),
    ...
    def _create_default_nfrs(self, category: str, scenarios: List[UserScenario], 
                           pain_points: List[PainPoint]) -> List[NonFunctionalRequirement]:
        ...
        default_nfrs = {
            'performance': NonFunctionalRequirement(
                ...
                acceptance_criteria=['Page loads within 2 seconds', 'Search results appear within 1 second'],
                source_personas=[s.persona_id for s in scenarios],
                source_scenarios=[s.id for s in scenarios]
            ),
            'usability': NonFunctionalRequirement(
                ...
                priority='high',
                measurable_criteria=['Task completion rate > 90%', 'User satisfaction score > 4/5'],
                acceptance_criteria=['New users can complete basic tasks within 5 minutes'],
                source_personas=[s.persona_id for s in scenarios],
                source_scenarios=[s.id for s in scenarios]
            ),
            'security': NonFunctionalRequirement(
                ...
                priority='critical',
                measurable_criteria=['Zero data breaches', 'All data encrypted in transit and at rest'],
                acceptance_criteria=['User authentication required', 'Data access logged'],
                source_personas=[s.persona_id for s in scenarios],
                source_scenarios=[s.id for s in scenarios]
            )
        }
    ...
    def _create_personas_artifact(self, personas: List[UserPersona], domain: str, 
    ...
    def _create_scenarios_artifact(self, scenarios: List[UserScenario], 
                                 system_context: Dict[str, Any]) -> Artifact:
    ...
    def _create_nfrs_artifact(self, nfrs: List[NonFunctionalRequirement], 
                            system_constraints: Optional[Dict[str, Any]]) -> Artifact:
    ...
    def get_persona_summary(self, persona_id: str) -> Optional[Dict[str, Any]]:
        ...
        related_scenarios = [s for s in self.scenarios_created.values() if s.persona_id == persona_id]
        related_pain_points = [p for p in self.pain_points_identified.values() if p.persona_id == persona_id]
    ...
    def get_nfr_coverage_analysis(self) -> Dict[str, Any]:
        ...
        coverage_analysis = {
            ...
            "categories_missing": [cat for cat in self.nfr_categories if cat not in nfr_by_category],
            "category_distribution": {cat: len(nfrs) for cat, nfrs in nfr_by_category.items()},
    ...
    d
ef classify_nfr_priority(self, nfr: NonFunctionalRequirement, 
        ...
        if any(keyword in nfr.description.lower() for keyword in 
               ['compliance', 'regulation', 'legal', 'audit', 'gdpr', 'hipaa']):
            priority_score += 3
    ...
    def generate_nfr_acceptance_criteria(self, nfr: NonFunctionalRequirement,
                                       related_scenarios: List[UserScenario]) -> List[str]:
        """
    ...
    def assess_nfr_feasibility(self, nfr: NonFunctionalRequirement,
        ...
        if nfr.category == 'performance':
            if budget_constraint == 'high' and team_size == 'large':
                ...
                assessment["technical_risks"].append("May require infrastructure upgrades")
            ...
            if technical_constraints.get('infrastructure', 'basic') == 'advanced':
                ...
                assessment["implementation_complexity"] = "high"
                assessment["technical_risks"].append("May require infrastructure investment")
        
        # Add general recommendations
        if assessment["feasibility_score"] < 0.5:
            assessment["recommendations"].append("Consider phased implementation approach")
    ...
    def generate_nfr_test_scenarios(self, nfr: NonFunctionalRequirement) -> List[Dict[str, Any]]:
        """
        ...
        if nfr.category == 'performance':
            ...
            test_scenarios.extend([
                ...
                },
                {
                    ...
                    "success_criteria": ["WCAG 2.1 compliance", "Screen reader compatibility"],
            ...
            test_scenarios.extend([
                {
                    ...
                    "success_criteria": ["No critical vulnerabilities", "Data remains secure"],
                ...
                },
                {
                    ...
                    "test_type": "auth_test",
                    "success_criteria": ["Strong password enforcement", "Session management"],
    ...
    def create_nfr_implementation_plan(self, nfrs: List[NonFunctionalRequirement],
        ...
        """
        # Prioritize NFRs
        prioritized_nfrs = sorted(nfrs, key=lambda x: {
            'critical': 4, 'high': 3, 'medium': 2, 'low': 1
        }.get(x.priority, 1), reverse=True)
        ...
        critical_high_nfrs = [nfr for nfr in prioritized_nfrs if nfr.priority in ['critical', 'high']]
        if critical_high_nfrs:
            phases.append({
                ...
                "dependencies": [],
                "deliverables": ["Security framework", "Performance baseline", "Core reliability features"]
            })
            current_phase += 1
        ...
        if medium_nfrs:
            phases.append({
                ...
                "deliverables": ["Usability improvements", "Accessibility compliance", "Scalability features"]
            })
            current_phase += 1
        ...
        if low_nfrs:
            phases.append({
                ...
                "deliverables": ["Maintainability improvements", "Additional monitoring"]
            })
