...
"""
    ...
    def chat_with_customer(self, customer: Customer, stakeholder_type: str = "customer") -> Dict[str, Any]:
        """
        ...
        while turn_count < self.max_customer_turns:
            ...
            action_prompt = self._get_action_prompt(
                ...
                context={
                    ...
                    "previous_responses": [turn.get("answer", "") for turn in interview_record["turns"][-3:]]
                }
            )
            ...
            question, reasoning, methodology, end_conversation = self.parse_response(response)
            ...
            extracted_requirements = self._extract_requirements_from_answer(answer, question)
            ...
            if end_conversation or self._should_end_conversation(interview_record):
                ...
                self.add_to_memory("assistant", "Ending conversation - requirements gathering complete.")
                break
    ...
    def _extract_requirements_from_answer(self, answer: str, question: str) -> List[Dict[str, Any]]:
        """
        ...
        requirement_indicators = [
            ...
            "system should", "application must", "user needs",
            "business requires", "compliance", "regulation"
        ]
    ...
    def _should_end_conversation(self, interview_record: Dict[str, Any]) -> bool:
        ...
        recent_turns = interview_record["turns"][-3:] if len(interview_record["turns"]) >= 3 else []
        recent_requirements = [req for req in interview_record["requirements_identified"] 
                             if any(req["source_question"] in turn.get("question", "") for turn in recent_turns)]
        
        if len(recent_requirements) == 0 and len(interview_record["turns"]) > 10:
            return True  # No new requirements in recent turns
        
        # Check completeness score
        completeness = self.assess_requirement_completeness(interview_record["requirements_identified"])
        ...
        def
 generate_follow_up_questions(self, previous_answers: List[str], context: Dict[str, Any]) -> List[str]:
        """
        ...
        if w5h1_module:
            ...
            for dimension, info in framework.items():
                if "questions" in info:
                    ...
                    follow_up_questions.extend(relevant_questions[:2])  # Limit to 2 per dimension
        
        # Apply Socratic questioning for deeper insights
        socratic_questions = self._generate_socratic_questions(previous_answers, context)
    ...
    def _select_relevant_questions(self, questions: List[str], previous_answers: List[str], 
    ...
    def _generate_socratic_questions(self, previous_answers: List[str], 
                                   context: Dict[str, Any]) -> List[str]:
        ...
        for answer in previous_answers:
            ...
            if "should" in answer.lower() or "must" in answer.lower():
                socratic_questions.append(f"What would happen if that requirement wasn't met?")
    ...
    def assess_requirement_completeness(self, requirements: List[Dict[str, Any]]) -> float:
        """
        ...
        functional_reqs = [req for req in requirements if req.get("type") == "functional"]
        if len(functional_reqs) >= 5:  # Minimum threshold
            score += completeness_criteria["functional_requirements"]
        else:
            score += completeness_criteria["functional_requirements"] * (len(functional_reqs) / 5)
        ...
        if len(nfr_reqs) >= 3:
            ...
            score += completeness_criteria["non_functional_requirements"] * (len(nfr_reqs) / 3)
        
        # Check for constraints
        constraint_keywords = ["must", "cannot", "limited", "restricted", "compliance"]
        constraint_reqs = [req for req in requirements 
                          if any(keyword in req.get("text", "").lower() for keyword in constraint_keywords)]
        ...
        if len(requirements) >= 10:  # Assume good stakeholder coverage with many requirements
            score += completeness_criteria["stakeholder_coverage"]
        else:
            score += completeness_criteria["stakeholder_coverage"] * (len(requirements) / 10)
        
        # Check business objectives coverage
        business_keywords = ["business", "objective", "goal", "value", "benefit", "roi"]
        business_reqs = [req for req in requirements 
                        if any(keyword in req.get("text", "").lower() for keyword in business_keywords)]
        if len(business_reqs) >= 2:
            score += completeness_criteria["business_objectives"]
        else:
            score += completeness_criteria["business_objectives"] * (len(business_reqs) / 2)
        
        # Check acceptance criteria
        acceptance_keywords = ["accept", "criteria", "test", "verify", "validate"]
        acceptance_reqs = [req for req in requirements 
                          if any(keyword in req.get("text", "").lower() for keyword in acceptance_keywords)]
        if len(acceptance_reqs) >= 1:
            score += completeness_criteria["acceptance_criteria"]
        
        return min(score, 1.0)  # Cap at 1.0
    
    def identify_requirement_gaps(self, requirements: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        if "non_functional" not in req_types:
            gaps.append("Missing non-functional requirements (performance, security, usability)")
        
        # Check for missing 5W1H coverage
        requirement_text = " ".join([req.get("text", "") for req in requirements]).lower()
    ...
    def get_interview_summary(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        summary = {
            ...
            "duration_minutes": (record["end_time"] - record["start_time"]).total_seconds() / 60,
            "total_turns": record["total_turns"],
            "requirements_count": len(record["requirements_identified"]),
            "completeness_score": record["completeness_score"],
            "gaps_identified": self.identify_requirement_gaps(record["requirements_identified"]),
            "key_requirements": record["requirements_identified"][:5],  # Top 5 requirements
            "status": record["status"]
        }
        
        return summary
    
    def create_stakeholder_profile(self, stakeholder_type: str, interview_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        sorted_types = sorted(requirement_types.items(), key=lambda x: x[1], reverse=True)
...
class Interviewer(InterviewerAgent):
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.max_enduser_turns = config.get('max_enduser_turns', 50)    def 
conduct_stakeholder_interview(self, stakeholder_info: Dict[str, Any], 
                                    interview_objectives: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Starting structured interview with {stakeholder_type} ({stakeholder_role})")
        
        # Create comprehensive interview record
        interview_record = {
            "session_id": session_id,
            "stakeholder_info": stakeholder_info,
            "interview_objectives": interview_objectives or self._get_default_objectives(),
            "start_time": datetime.now(),
            "phases": [],
        ...
        interview_record["completeness_assessment"] = self._assess_interview_completeness(interview_record)
        interview_record["quality_score"] = self._calculate_interview_quality_score(interview_record)
        
        logger.info(f"Completed structured interview {session_id} with quality score: {interview_record['quality_score']}")
    ...
    def _conduct_context_exploration_phase(self, interview_record: Dict[str, Any]) -> None:
        """Conduct business context exploration phase."""
        phase_data = {
            "phase_name": "context_exploration",
            "start_time": datetime.now(),
            "objectives": ["Understand business domain", "Identify key processes", "Discover pain points"],
            "questions_asked": [],
            "insights_gathered": []
        }
    ...
    def _conduct_systematic_elicitation_phase(self, interview_record: Dict[str, Any]) -> None:
        ...
        if w5h1_module:
            ...
            for dimension, info in framework.items():
                dimension_questions = info.get("questions", [])[:3]  # Limit to 3 per dimension
    ...
    def _conduct_deep_dive_phase(self, interview_record: Dict[str, Any]) -> None:
        ...
        phase_data = {
            ...
            "start_time": datetime.now(),
            "objectives": ["Deep exploration of critical areas", "Challenge assumptions"],
            "questions_asked": [],
            "insights_gathered": []
        }
        ...
        socratic_questions = self._generate_socratic_questions(previous_insights, {
            "stakeholder_info": interview_record["stakeholder_info"]
        })
    ...
    def _conduct_validation_phase(self, interview_record: Dict[str, Any]) -> None:
        ...
        phase_data = {
            ...
            "start_time": datetime.now(),
            "objectives": ["Validate understanding", "Identify gaps", "Confirm priorities"],
            "questions_asked": [],
            "insights_gathered": []
        }
    ...
    def _conduct_closure_phase(self, interview_record: Dict[str, Any]) -> None:
        ...
        phase_data = {
            "phase_name": "closure",
            "start_time": datetime.now(),
            "objectives": ["Summarize findings", "Define next steps", "Thank stakeholder"],
            "questions_asked": [],
            "insights_gathered": []
        }
    ...
    def _assess_interview_completeness(self, interview_record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_score < 0.7:
            assessment["recommendations"].append("Consider additional interview sessions")
        if assessment["dimension_scores"].get("non_functional_requirements", 0) < 0.5:
            assessment["recommendations"].append("Focus more on non-functional requirements")
    ...
    def _assess_dimension_completeness(self, interview_record: Dict[str, Any], dimension: str) -> float:
        ...
        if dimension == "functional_requirements":
            functional_reqs = [req for req in requirements if req.get("type") == "functional"]
            return min(len(functional_reqs) / 8.0, 1.0)  # Target: 8 functional requirements
            ...
            stakeholder_mentions = sum(1 for phase in phases if "stakeholder" in str(phase).lower())
            return min(stakeholder_mentions / 3.0, 1.0)  # Target: 3 stakeholder discussions
        
        elif dimension == "business_context":
            context_phases = [p for p in phases if p.get("phase_name") == "context_exploration"]
            ...
            return min((len(constraints) + len(assumptions)) / 4.0, 1.0)  # Target: 4 total
        
        elif dimension == "validation_and_gaps":
            validation_phases = [p for p in phases if p.get("phase_name") == "validation"]
            gaps = interview_record.get("gaps_identified", [])
            return 1.0 if validation_phases and gaps else 0.5
        
        return 0.0
    
    def _calculate_interview_quality_score(self, interview_record: Dict[str, Any]) -> float:
        """Calculate overall interview quality score."""
        completeness = interview_record.get("completeness_assessment", {}).get("overall_score", 0.0)
        ...
        requirements_score = min(requirements_count / 10.0, 1.0)  # Target: 10 requirements
        ...
        return round(quality_score, 2)    def
 create_interview_artifact(self, interview_record: Dict[str, Any]) -> Artifact:
        """
        ...
        metadata = ArtifactMetadata(
            ...
            author=self.name,
            version="1.0",
            tags=["interview", "requirements", "elicitation", interview_record['stakeholder_info'].get('type', 'stakeholder')],
        ...
        artifact_content = {
            "interview_metadata": {
                ...
                "completeness_score": interview_record.get("completeness_assessment", {}).get("overall_score", 0.0)
            },
            "interview_process": {
                "objectives": interview_record.get("interview_objectives", []),
                "phases_completed": [phase["phase_name"] for phase in interview_record.get("phases", [])],
                "total_questions_asked": sum(len(phase.get("questions_asked", [])) for phase in interview_record.get("phases", [])),
                "methodologies_applied": list(set([
                    q.get("methodology", "") for phase in interview_record.get("phases", [])
                    for q in phase.get("questions_asked", [])
                ]))
            },
            "requirements_discovered": {
                "functional_requirements": [req for req in interview_record.get("requirements_identified", []) if req.get("type") == "functional"],
                "non_functional_requirements": [req for req in interview_record.get("requirements_identified", []) if req.get("type") == "non_functional"],
                "constraints": interview_record.get("constraints_identified", []),
                "assumptions": interview_record.get("assumptions_identified", []),
            ...
            "analysis_results": {
                ...
                "stakeholder_concerns": interview_record.get("stakeholder_concerns", []),
            ...
            "raw_interview_data": {
                "phases": interview_record.get("phases", []),
                "start_time": interview_record.get("start_time").isoformat() if interview_record.get("start_time") else None,
                "end_time": interview_record.get("end_time").isoformat() if interview_record.get("end_time") else None
            }
        }
        ...
        logger.info(f"Created interview artifact {artifact.id} for session {interview_record['session_id']}")
        
        return artifact
    
    def extract_initial_requirements(self, interview_record: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        for req in interview_record.get("requirements_identified", []):
            structured_req = {
                ...
                "source": {
                    "type": "interview",
                    "session_id": interview_record["session_id"],
                    "stakeholder": interview_record["stakeholder_info"].get("type", "unknown"),
                    "question": req.get("source_question", ""),
                    "extracted_at": req.get("extracted_at").isoformat() if req.get("extracted_at") else None
                },
                "acceptance_criteria": self._generate_acceptance_criteria(req.get("text", "")),
                "business_value": self._assess_business_value(req.get("text", "")),
                "complexity": self._assess_complexity(req.get("text", "")),
                "dependencies": [],
                "assumptions": [],
                "constraints": [],
                "verification_method": self._suggest_verification_method(req.get("text", "")),
                "status": "draft",
        ...
        for constraint in interview_record.get("constraints_identified", []):
            constraint_req = {
                ...
                "acceptance_criteria": [f"System must comply with: {constraint.get('description', '')}"],
        ...
        logger.info(f"Extracted {len(initial_requirements)} initial requirements from interview {interview_record['session_id']}")
    ...
    def _assess_business_value(self, requirement_text: str) -> str:
        ...
        high_value_keywords = ["revenue", "cost", "efficiency", "compliance", "security", "critical"]
        medium_value_keywords = ["user experience", "performance", "quality", "productivity"]
    ...
    def _assess_complexity(self, requirement_text: str) -> str:
        ...
        high_complexity_keywords = ["integration", "algorithm", "real-time", "distributed", "machine learning"]
        medium_complexity_keywords = ["database", "api", "workflow", "calculation", "validation"]
    ...
    def _suggest_verification_method(self, requirement_text: str) -> str:
        ...
        if "performance" in requirement_text.lower() or "speed" in requirement_text.lower():
            return "performance_testing"
        elif "security" in requirement_text.lower() or "authentication" in requirement_text.lower():
            return "security_testing"
        elif "user" in requirement_text.lower() or "interface" in requirement_text.lower():
            return "user_acceptance_testing"
        elif "integration" in requirement_text.lower() or "api" in requirement_text.lower():
    ...
    def _identify_risk_factors(self, interview_record: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Identify potential risk factors from interview."""
        risk_factors = []
        
        # Check for complexity indicators
        requirements = interview_record.get("requirements_identified", [])
        complex_reqs = [req for req in requirements if self._assess_complexity(req.get("text", "")) == "high"]
        ...
        integration_reqs = [req for req in requirements if "integration" in req.get("text", "").lower()]
        if integration_reqs:
            risk_factors.append({
    ...
    def integrate_with_artifact_pool(self, interview_record: Dict[str, Any], 
                                   artifact_pool=None) -> Dict[str, str]:
        """
        ...
        if initial_requirements:
            ...
            requirements_artifact = Artifact(
                ...
                content={
                    ...
                    "extraction_metadata": {
                        ...
                        "functional_count": len([r for r in initial_requirements if r["type"] == "functional"]),
                        "non_functional_count": len([r for r in initial_requirements if r["type"] == "non_functional"]),
                        "constraint_count": len([r for r in initial_requirements if r["type"] == "constraint"]),
            ...
            if artifact_pool:
                requirements_artifact_id = artifact_pool.store_artifact(requirements_artifact)
        ...
        logger.info(f"Integrated interview results with artifact pool: {list(created_artifacts.keys())}")
        
        return created_artifacts
    
    def generate_interview_summary_report(self, interview_record: Dict[str, Any]) -> str:
        """
        ...
        report_lines.append(f"Stakeholder Type: {interview_record['stakeholder_info'].get('type', 'Unknown')}")
        report_lines.append(f"Stakeholder Role: {interview_record['stakeholder_info'].get('role', 'Unknown')}")
        report_lines.append(f"Duration: {interview_record.get('total_duration_minutes', 0):.1f} minutes")
        report_lines.append(f"Quality Score: {interview_record.get('quality_score', 0.0):.2f}")
        report_lines.append(f"Completeness Score: {interview_record.get('completeness_assessment', {}).get('overall_score', 0.0):.2f}")
        report_lines.append("")
        ...
        for phase in phases:
            report_lines.append(f"  - {phase['phase_name'].replace('_', ' ').title()}: {len(phase.get('questions_asked', []))} questions")
        report_lines.append("")
        ...
        report_lines.append(f"  - Constraints: {len(interview_record.get('constraints_identified', []))}")
        report_lines.append(f"  - Assumptions: {len(interview_record.get('assumptions_identified', []))}")
        report_lines.append("")
        ...
        report_lines.append("=" * 60)
        report_lines.append(f"Report generated by {self.name} on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report_lines.append("=" * 60)
