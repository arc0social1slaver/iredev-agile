...
class AdvancedTokenRateLimiter(BaseRateLimiter):
    ...
    @classmethod
    def from_config(cls, provider: str, config: Dict[str, Any]) -> "AdvancedTokenRateLimiter":
        ...
        return cls(
            ...
            input_token_price_per_million=config.get("input_token_price_per_million", 0.0),
            output_token_price_per_million=config.get("output_token_price_per_million", 0.0),
        )
    ...
    def wait_if_needed(self, input_tokens: int, estimated_output_tokens: Optional[int] = None) -> None:
        """
    ...
    def _can_proceed(self, in_tk: int, out_tk: int) -> bool:
        ...
        return (
            ...
            and self._window_sum(self.input_token_usage) + in_tk <= self.input_tokens_per_minute
            and self._window_sum(self.output_token_usage) + out_tk <= self.output_tokens_per_minute
        )
        ...
        with self._lock:
            ...
            while True:
                ...
                if (
                        ...
                        and (current_input_tokens + input_tokens) <= self.input_tokens_per_minute
                        and (current_output_tokens + estimated_output_tokens) <= self.output_tokens_per_minute
                ):
                    break

                wait_time = 0.0
                if self.request_timestamps:
                    wait_time = max(wait_time, 60.0 - (now - self.request_timestamps[0]))
                if self.input_token_usage:
                    wait_time = max(wait_time, 60.0 - (now - self.input_token_usage[0][0]))
                if self.output_token_usage:
                    wait_time = max(wait_time, 60.0 - (now - self.output_token_usage[0][0]))
                ...
                logger.info(f"[{self.provider}] Rate limit approaching. Waiting {wait_time:.2f} seconds...")
