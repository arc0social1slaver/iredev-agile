...
from .base_types import (
    KnowledgeType, KnowledgeModule, KnowledgeModuleConfig, KnowledgeMetadata,
    KnowledgeDependency, ModuleStatus, LoadPriority, KnowledgeQuery,
    KnowledgeException, ModuleNotFoundException, ModuleLoadException,
    DependencyException, ValidationException
)
...
class KnowledgeManager:
    ...
    def _initialize_paths(self) -> None:
        ...
        self._type_paths = {
            KnowledgeType.DOMAIN_KNOWLEDGE: Path(self.config.get("domain_knowledge_path", "knowledge/domains")),
            KnowledgeType.METHODOLOGY: Path(self.config.get("methodology_path", "knowledge/methodologies")),
            KnowledgeType.STANDARDS: Path(self.config.get("standards_path", "knowledge/standards")),
            KnowledgeType.TEMPLATES: Path(self.config.get("templates_path", "knowledge/templates")),
            KnowledgeType.STRATEGIES: Path(self.config.get("strategies_path", "knowledge/strategies"))
        }
        ...
        for knowledge_type, type_path in self._type_paths.items():
            ...
            if config_file.exists():
                try:
                    ...
                    for module_id, module_config in modules_config.get("modules", {}).items():
                        config_obj = KnowledgeModuleConfig(
                            module_id=module_id,
                            module_type=knowledge_type,
                            file_path=str(type_path / module_config.get("file", f"{module_id}.yaml")),
                    ...
                    logger.error(f"Failed to load module configuration from {config_file}: {str(e)}")
            else:
                # Create default configuration file
                self._create_default_module_config(knowledge_type, config_file)
    
    def _create_default_module_config(self, knowledge_type: KnowledgeType, config_file: Path) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to create default configuration {config_file}: {str(e)}")
    
    def _get_default_modules_for_type(self, knowledge_type: KnowledgeType) -> Dict[str, Any]:
    ...
    def load_module(self, module_id: str, force_reload: bool = False) -> Optional[KnowledgeModule]:
        ...
        if not force_reload and self.cache_enabled and module_id in self._modules:
            ...
            if cached_module.is_valid():
                ...
                logger.warning(f"Cached module {module_id} failed integrity check, reloading")
        ...
        for dep_id in config.dependencies:
            if dep_id not in self._modules:
                ...
                if not dep_module:
                    logger.error(f"Failed to load dependency {dep_id} for module {module_id}")
                    return None
        ...
        try:
            ...
            if isinstance(e, (ModuleNotFoundException, ValidationException, KnowledgeException)):
                raise
            logger.error(f"Failed to load module {module_id}: {str(e)}")
            raise ModuleLoadException(f"Failed to load module {module_id}: {str(e)}")
    
    def load_modules_by_type(self, knowledge_type: KnowledgeType, force_reload: bool = False) -> List[KnowledgeModule]:
    ...
    def load_modules_for_agent(self, agent_name: str, module_names: List[str], force_reload: bool = False) -> List[KnowledgeModule]:
        ...
        for module_name in module_names:
            ...
            if module:
                ...
                logger.warning(f"Failed to load module {module_name} for agent {agent_name}")
    ...
    def list_available_modules(self, knowledge_type: Optional[KnowledgeType] = None) -> List[Dict[str, Any]]:
        ...
        for module_id, dependencies in self._dependency_graph.items():
            for dep_id in dependencies:
                if dep_id not in self._module_configs:
                    ...
                    errors.append(f"Module {module_id} depends on disabled module {dep_id}")
    ...
    def _matches_filters(self, module: KnowledgeModule, filters: Dict[str, Any]) -> bool:
        ...
        for field, value in filters.items():
            if field == "module_type" and module.module_type != value:
                ...
                return False
            elif field == "tags" and not any(tag in module.metadata.tags for tag in value):
                return False
    ...
    def update_module(self, module_id: str, new_content: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not self._validate_module(module):
                raise ValidationException(f"Updated module validation failed: {module_id}")
    ...
    def create_module_template(self, knowledge_type: KnowledgeType, module_id: str) -> str:
