...
class HumanReviewManager:
    ...
    def __init__(self, artifact_pool: ArtifactPool, event_bus: EventBus):
        ...
        self.on_review_completed: Optional[Callable[[ReviewPoint, HumanFeedback], None]] = None
    ...
    def create_review_point(self, session_id: str, artifact_id: str, 
                           phase: str, description: str,
                           timeout_minutes: Optional[int] = None,
                           priority: int = 1,
                           assigned_reviewer: Optional[str] = None) -> ReviewPoint:
        """
        ...
        with self._lock:
            ...
            self.artifact_pool.update_artifact(artifact_id, {'status': ArtifactStatus.UNDER_REVIEW.value})
            ...
            logger.info(f"Created review point {review_point.id} for artifact {artifact_id}")
            
            return review_point
    
    def submit_feedback(self, review_point_id: str, reviewer: str,
                       feedback_type: FeedbackType, content: str,
                       suggestions: Optional[List[str]] = None,
                       approval_status: bool = False,
                       confidence_score: Optional[float] = None) -> HumanFeedback:
        """
        ...
        with self._lock:
            ...
            if not review_point:
                raise ValueError(f"Review point {review_point_id} not found or already completed")
            ...
            logger.info(f"Received feedback for review point {review_point_id} from {reviewer}")
            
            return feedback
    
    def get_pending_reviews(self, reviewer: Optional[str] = None,
                           session_id: Optional[str] = None) -> List[ReviewPoint]:
        """
        ...
        with self._lock:
            ...
            # Filter by reviewer
            if reviewer:
                reviews = [r for r in reviews if r.assigned_reviewer == reviewer or r.assigned_reviewer is None]
    ...
    def get_review_statistics(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if session_id:
                active = [r for r in self.active_reviews.values() if r.session_id == session_id]
                completed = [r for r in self.completed_reviews.values() if r.session_id == session_id]
            else:
            ...
            completed_count = len([r for r in completed if r.status == ReviewStatus.COMPLETED])
            timeout_count = len([r for r in completed if r.status == ReviewStatus.TIMEOUT])
            cancelled_count = len([r for r in completed if r.status == ReviewStatus.CANCELLED])
            ...
            if completed_count > 0:
                ...
                for review in completed:
                    if review.status == ReviewStatus.COMPLETED:
                        feedback_list = self.feedback_history.get(review.id, [])
                        if feedback_list:
                            response_time = (feedback_list[0].created_at - review.created_at).total_seconds()
            ...
            return {
                ...
                'completion_rate': completed_count / total_reviews if total_reviews > 0 else 0.0
            }
