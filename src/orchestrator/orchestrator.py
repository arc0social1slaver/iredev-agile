...
class ProjectConfig:
    ...
    review_points: List[str] = field(default_factory=lambda: ["url_generation", "model_creation", "srs_generation"])
...
class RequirementOrchestrator:
    ...
    def __init__(self, config_manager: ConfigManager, 
        ...
        self.on_review_required: Optional[Callable[[str, str, str], None]] = None  # session_id, artifact_type, artifact_id
    ...
    def start_requirement_process(self, project_config: ProjectConfig, created_by: str) -> ProcessSession:
        """
        ...
        with self._lock:
            ...
            coordinator.on_task_completed = lambda task_id, agent_name: self._on_task_completed(
                session.session_id, task_id, agent_name
            )
            coordinator.on_task_assigned = lambda task_id, agent_name: self._on_task_assigned(
                session.session_id, task_id, agent_name
            )
            ...
            logger.info(f"Started requirement process for project: {project_config.project_name}")
            
            # Start the orchestration process
            asyncio.create_task(self._run_orchestration(session.session_id))
            
            return session
    
    def pause_for_human_review(self, session_id: str, artifact_type: str, artifact_id: str) -> None:
        """
        ...
        with self._lock:
            ...
            if self.on_review_required:
                self.on_review_required(session_id, artifact_type, artifact_id)
            
            logger.info(f"Paused session {session_id} for review of {artifact_type}: {artifact_id}")
    ...
    async def _run_orchestration(self, session_id: str) -> None:
        ...
        try:
            ...
            while (session.status == ProcessStatus.RUNNING and 
                   session.current_phase != ProcessPhase.COMPLETED and
    ...
    async def _execute_phase(self, session_id: str) -> None:
        ...
        logger.info(f"Executing phase {session.current_phase.value} for session {session_id}")
    ...
    async def _handle_review_phase(self, session_id: str) -> None:
        ...
        artifacts = self.artifact_pool.query_artifacts_by_type(artifact_type, session_id)
        
        if not artifacts:
            logger.error(f"No artifacts found for review in phase {session.current_phase.value}")
            return
        ...
        # Pause for review
        self.pause_for_human_review(session_id, artifact_type.value, latest_artifact.id)
    ...
    def _create_phase_task(self, session: ProcessSession, agent_name: str) -> Task:
        ...
        return Task(
            ...
            description=task_descriptions.get(session.current_phase, f"Execute {session.current_phase.value}"),
    ...
    async def _transition_to_phase(self, session_id: str, next_phase: ProcessPhase) -> None:
        """Transition to the next phase."""
        session = self.active_sessions.get(session_id)
        if not session:
            return
        
        logger.info(f"Transitioning session {session_id} from {session.current_phase.value} to {next_phase.value}")
    ...
    def _on_task_completed(self, session_id: str, task_id: str, agent_name: str) -> None:
